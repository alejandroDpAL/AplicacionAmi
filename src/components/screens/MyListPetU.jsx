...
const MyListPetU = ({ route, navigation }) => {
  ...
  const fetchAdoptedPets = async () => {
    try {
      ...
      if (identificacion) {
        const response = await axiosClient.get(`/mascota/proceso/${identificacion}`);
        if (response.data && response.data.mascotas) {
          ...
          Alert.alert('Error', 'No se encontraron mascotas.');
        }
        setIsLoading(false);
      } else {
  ...
  const denyAdoption = async (id_mascota) => {
    try {
      const response = await axiosClient.post(`/mascota/solicitud/${id_mascota}`, {
        accion: 'denegar'
      });

      if (response.status === 200) {
        Alert.alert('solicitud cancelada con exito');
