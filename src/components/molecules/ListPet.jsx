...
const ListPet = ({ visible, onClose, pet, refreshPets }) => {
  // Añadir refreshPets para actualizar la lista de mascotas
      ...
      if (pet && pet.id_mascota) {
        try {
          ...
          if (error.response && error.response.status === 403) {
            setVacunasError("No hay vacunas para esta mascota.");
            setVacunas([]);
          } else {
  ...
  const handleAdoptar = async () => {
    try {
      ...
      const response = await axiosClient.post(
        `/mascota/iniciar/${pet.id_mascota}`,
        { id_usuario }
      );
      if (response.status === 200) {
        Alert.alert("Éxito", "Solicitud de adopción enviada");
        ...
        <View style={styles.modalContent}>
          <ScrollView contentContainerStyle={styles.scrollViewContainer}>
            ...
            <Text style={styles.modalSubtitle}>
              Esterilización: {pet.esterilizacion}
            </Text>
            ...
            {vacunasError ? (
              ...
              <View style={styles.vacunaRowContainer}>
                {vacunas.map((vacuna, index) => (
                  <View key={vacuna.id_vacuna} style={styles.vacunaContainer}>
                    ...
                    <Text style={styles.modalSubtitle}>
                      Fecha: {formatDate(vacuna.fecha_vacuna)}
                    </Text>
                    <Text style={styles.modalSubtitle}>
              ...
              <Text style={styles.noVacunasText}>
          ...
          <View style={styles.buttonContainer}>
            ...
            >
              <Text style={styles.buttonText}>¡Adóptar!</Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  modalImage: {
    width: 500, // Cambiar el tamaño de la imagen
    height: 500, // Cambiar el tamaño de la imagen
    borderRadius: 10,
    marginBottom: 15,
  },
  modalDescription: {
  ...
  buttonContainer: {
    flexDirection: "row", // Alinear botones horizontalmente
    justifyContent: "space-between", // Separar botones
